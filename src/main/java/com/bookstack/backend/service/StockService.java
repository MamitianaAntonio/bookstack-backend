...
public class StockService {
    ...
    public BookStockResponseDTO getBookStock(String bookId){
        JBook book = bookRepository.findById(bookId)
                .orElseThrow(() -> new NotFoundException("Book with Id : "+ bookId +" not found"));

        List<JBookCopy> bookCopies = bookCopyRepository.findByBookId(bookId);

        List<FormatStockDTO> formatStock = bookCopies.stream()
                .map(copy -> {
    ...
    public FormatStockDTO getFormatStock(String bookCopyId){
        JBookCopy copy = bookCopyRepository.findById(bookCopyId)
                .orElseThrow(() -> new NotFoundException("BookCopy with Id : "+ bookCopyId +" not found"));
